...
with tab2:
    ...
    with st.container(border=True):
        ...
        with col2:
            ...
            月经周期 = st.select_slider('月经周期', ['<21天', '21-27天', '28-35天', '36-60天', '61-90天', '>90天'])
    ...
    with st.container(border=True):
        ...
        with col3:
            ...
            月经色 = st.select_slider('月经色（淡红, 正常, 黯褐）', ['淡红', '正常', '黯褐'])
            月经质 = st.select_slider('月经质地（稀薄, 正常, 稠浓）', ['稀薄', '正常', '稠浓'])
            月经伴血块 = st.toggle('月经血块')
            经间期出血 = st.toggle('经间期出血', help='经间期出血是指以氤氲期(即排卵期)周期性出现子宫少量出血。')
            经血崩漏 = st.toggle('经血崩漏')
        ...
        with col4:
            ...
            痛经时痛连腰际 = st.toggle('痛经时痛连腰际')
            痛经时腹坠痛空痛隐痛 = st.toggle('痛经时腹坠痛/空痛/隐痛')
            痛经时腹痛喜温喜按 = st.toggle('痛经时腹痛喜温喜按')
            痛经时腹满胀痛无定处 = st.toggle('痛经时腹满胀痛无定处')
            贪食生冷或遇寒后加重月经不适 = st.toggle('贪食生冷或遇寒会加重月经不适')
            情绪失调时加重月经不适 = st.toggle('情绪失调时会加重月经不适')
            劳累后加重月经不适 = st.toggle('劳累后会加重月经不适')
            带下量 = st.select_slider('带下量（减少, 正常, 增加）', ['减少', '正常', '增加'])
            带下色 = st.select_slider('带下色（白, 正常, 黄）', ['白', '正常', '黄'])
            带下质 = st.select_slider('带下质（稀薄, 正常, 浓稠）', ['稀薄', '正常', '浓稠'])
    ...
    with st.container(border=True):
        ...
        with col12:
            舌边尖红 = st.toggle('舌边尖红')
            舌苔厚薄 = st.select_slider('舌苔薄厚', ['薄', '厚'])
            舌苔腻 = st.toggle('舌苔腻')
            舌苔白 = st.toggle('舌苔白')
            舌苔黄 = st.toggle('舌苔黄')
            舌苔黄白 = st.toggle('舌苔黄白相间')
    ...
    with st.container(border=True):
        ...
        with col14:
            ...
            脉涩 = st.toggle('脉涩')
            脉无力 = st.toggle('脉无力')
        脉诊参数 = st.checkbox("脉诊参数")
        if 脉诊参数:
            col13b, col14b  = st.columns(2)
            with col13b:
                st.markdown("**左手脉诊客观化参数**",)
                左H1 = st.number_input("左H1", 0.0000, 5000.0000, 0.0001)
                左H2 = st.number_input("左H2", 0.0000, 5000.0000, 0.0001)
                左H4 = st.number_input("左H4", 0.0000, 5000.0000, 0.0001)
                左H5 = st.number_input("左H5", 0.0000, 5000.0000, 0.0001)
                左T = st.number_input("左T", 0.0000, 5000.0000, 0.0001)
                左T1 = st.number_input("左T1", 0.0000, 5000.0000, 0.0001)
                左T4 = st.number_input("左T4", 0.0000, 5000.0000, 0.0001)
                左T5 = st.number_input("左T5", 0.0000, 5000.0000, 0.0001)
            with col14b:
                st.markdown("**右手脉诊客观化参数**",)
                右H1 = st.number_input("右H1", 0.0000, 5000.0000, 0.0001)
                右H2 = st.number_input("右H2", 0.0000, 5000.0000, 0.0001)
                右H4 = st.number_input("右H4", 0.0000, 5000.0000, 0.0001)
                右H5 = st.number_input("右H5", 0.0000, 5000.0000, 0.0001)
                右T = st.number_input("右T", 0.0000, 5000.0000, 0.0001)
                右T1 = st.number_input("右T1", 0.0000, 5000.0000, 0.0001)
                右T4 = st.number_input("右T4", 0.0000, 5000.0000, 0.0001)
                右T5 = st.number_input("右T5", 0.0000, 5000.0000, 0.0001)
    ...
    if 脉诊参数:
        左H2H1 = 左H2 / 左H1 if 左H1 != 0 else None
        左T1T = 左T1 / 左T if 左T != 0 else None
        左T4T = 左T4 / 左T if 左T != 0 else None
        左T1T4 = 左T1 / 左T4 if 左T4 != 0 else None
        右T4T = 右T4 / 右T if 右T != 0 else None
    ...
    if st.button("提交并预测证型", type='primary') == 1:
        ...
        if ((身高==0) | (体重==0)):
            ...
            if 脉诊参数:
                #Creating a feature list for the prediction model with pulse features
                feature_list = [左H2H1, 左T1T, 左T4T, 左T1T4, 右T4T, 体重, BMI, 月经初潮, 研究生, 月经量明显增多, 月经伴血块,
                                月经色黯滞, 月经期乳房胀痛, 月经期腹泻, 月经期情志异常, 痛经, 痛经时腹痛喜温喜按, 情绪失调时加重月经不适,
                                贪食生冷或遇寒后加重月经不适, 劳累后加重月经不适, 带下量多, 畏寒, 畏热, 肢冷, 手足心热, 自汗, 盗汗, 胸闷,
                                乏力懒言, 耳鸣, 喉中有痰, 泛酸, 胃脘胀满, 善叹息, 腰膝酸软, 口渴喜饮, 口渴不欲饮或少饮, 口苦, 口干, 便溏,
                                便秘, 失眠, 多梦, 情志不遂, 面色有泽, 上额痤疮, 唇甲青紫, 舌苔白, 舌色红, 青紫舌, 舌齿痕, 胖舌, 舌裂纹,
                                舌瘀斑, 舌苔腻, 舌苔白]
                # Reshaping the feature list into a format suitable for model input
                single_sample = np.array(feature_list).reshape(1, -1)
                # Load the model and make a prediction
                loaded_model = pickle.load(open('xgb_pulse_model.pkl', 'rb'))
            else:
                # Creating a feature list for the prediction model
                feature_list = [B型, O型, 家庭主妇, 中学以下, 剖宫产, 流产, 初潮_11_14, 初潮_14, 月经量明显增多, 月经伴血块, 经间期出血,
                                月经色黯滞, 月经质稀薄, 月经期乳房胀痛, 月经期腹泻, 月经期情志异常,
                                痛经, 痛经时痛连腰际, 痛经时腹痛喜温喜按, 痛经时腹满胀痛无定处,
                                情绪失调时加重月经不适, 贪食生冷或遇寒后加重月经不适, 劳累后加重月经不适,
                                带下正常, 带下量多, 带下色白, 带下色黄, 带下质稠,
                                畏寒, 肢冷, 潮热, 手足心热, 盗汗, 心悸, 胸闷, 乏力懒言, 喉中有痰, 胃脘胀满,
                                善叹息, 腰膝酸软, 口渴喜饮, 喜热饮, 口渴不欲饮或少饮, 纳呆, 口苦, 口干, 喜肉食动物油, 嗜冷饮,
                                便溏, 便秘, 大便干结, 失眠, 多梦, 寐迟, 情志不遂, 上额痤疮,
                                唇甲青紫, 舌色淡白, 舌色红, 青紫舌, 舌边尖红, 胖舌, 舌齿痕, 舌裂纹, 舌瘀斑, 舌苔厚, 舌苔腻,
                                舌苔白, 舌苔黄白, 脉沉, 脉数, 脉滑, 脉弦, 脉细]
                # Reshaping the feature list into a format suitable for model input
                single_sample = np.array(feature_list).reshape(1, -1)
                # Load the model and make a prediction
            ...
            肾阴虚证 = probabilities[0, 0]
            肾阳虚证 = probabilities[0, 1]
            ...
            if 脉诊参数:
                column_names = ['录入时间', '用户IP','姓名', '年龄', '身高', '体重', 'BMI','血型', '婚姻', '职业', '学历', '月经初潮', '经期', '月经周期', '怀孕', '顺产', '剖宫产', '流产',
                                '月经周期提前', '月经周期延后', '月经先后无定期', '闭经', '月经量',
                                '月经色', '月经质', '月经伴血块', '经间期出血', '经血崩漏', '月经期乳房胀痛', '月经期腹泻', '月经期情志异常',
                                '痛经', '痛经时痛连腰际', '痛经时腹坠痛空痛隐痛', '痛经时腹痛喜温喜按', '痛经时腹满胀痛无定处',
                                '贪食生冷或遇寒后加重月经不适', '情绪失调时加重月经不适', '劳累后加重月经不适', '带下量', '带下色', '带下质',
                                '畏寒', '畏热', '肢冷', '手足心热', '潮热', '自汗', '盗汗',
                                '心悸', '胸闷', '乏力懒言', '耳鸣', '喉中有痰', '泛酸', '胃脘胀满', '胁肋胀满疼痛', '腰膝酸软',
                                '善叹息', '纳呆', '口干', '口苦', '口渴喜饮', '喜热饮', '嗜冷饮', '口渴不欲饮或少饮', '喜肉食动物油',
                                '失眠', '多梦', '情志不遂', '规律运动', '多毛症',
                                '便溏', '完谷不化', '晨起泄泻', '便秘', '大便干结',
                                '小便清长', '小便黄赤', '小便频数', '小便量少', '夜尿频多',
                                '面色有泽', '上额痤疮', '两颊痤疮', '两颧红', '唇甲青紫',
                                '胖舌', '舌齿痕', '舌裂纹', '舌点刺', '舌瘀斑',
                                '舌色', '舌边尖红', '舌苔厚薄', '舌苔腻', '舌苔白', '舌苔黄', '舌苔黄白',
                                '脉浮', '脉沉', '脉迟', '脉数', '脉细', '脉滑', '脉弦', '脉涩', '脉无力',
                                '左H2H1', '左T1T', '左T4T', '左T1T4', '右T4T',
                                '多囊', '高血压', '高血脂', '糖尿病', '胰岛素抵抗',
                                '肾阴虚证', '肾阳虚证', '痰湿证', '血瘀证', '气滞证']

                save_list = [录入时间, 用户IP, 姓名, 年龄, 身高, 体重, BMI, 血型, 婚姻, 职业, 学历, 月经初潮, 经期, 月经周期, 怀孕, 顺产, 剖宫产, 流产,
                             月经周期提前, 月经周期延后, 月经先后无定期, 闭经, 月经量,
                             月经色, 月经质, 月经伴血块, 经间期出血, 经血崩漏, 月经期乳房胀痛, 月经期腹泻, 月经期情志异常,
                             痛经, 痛经时痛连腰际, 痛经时腹坠痛空痛隐痛, 痛经时腹痛喜温喜按, 痛经时腹满胀痛无定处,
                             贪食生冷或遇寒后加重月经不适, 情绪失调时加重月经不适, 劳累后加重月经不适, 带下量, 带下色, 带下质,
            ...
            if prediction[0,0] == 1:
                ...
                with col1:
                    st.success(f' {probabilities[0, 0]*100:.2f}%诊断为肾阴虚证')
                with col2:
            ...
            if prediction[0,1] == 1:
                col1, col2 = st.columns([1.5,2.5])
                with col1:
                    st.success(f' {probabilities[0, 1]*100:.2f}%诊断为肾阳虚证')
                with col2:
